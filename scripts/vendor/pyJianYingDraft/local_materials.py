...
class VideoMaterial:
    ...
    def export_json(self) -> Dict[str, Any]:
        # 检测视频是否包含音频轨道，设置 has_audio
        has_audio = False
        try:
            import pymediainfo
            info: pymediainfo.MediaInfo = pymediainfo.MediaInfo.parse(self.path, mediainfo_options={"File_TestContinuousFileNames": "0"})
            if len(info.audio_tracks) > 0:
                has_audio = True
        except Exception:
            # 如果检测失败，保守认为有音频
            has_audio = True if self.material_type == "video" else False
        ...
        video_material_json = {
            ...
            "source": 0,
            "source_platform": 0,
            "stable": {"matrix_path": "", "stable_level": 0, "time_range": None},
            "team_id": "",
            "type": self.material_type,
            "video_algorithm": {"algorithms": [], "complement_frame_config": None, "deflicker": None, "gameplay_configs": [], "motion_blur_config": None, "noise_reduction": None, "path": "", "quality_enhance": None, "time_range": None},
            "width": self.width
        }
